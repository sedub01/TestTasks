...
// Глобальная переменная для хранения данных ключ/значение
map<string, string> data;
mutex dataMutex;
int total_requests = 0;
int requests_last_5_seconds = 0;
    ...
    try {
        while (true) {
            ...
            if (message.substr(0, 4) == GET_KEY) {
                ...
                response = data.find(key) != data.end() ? data[key] : "Key not found";
...
int main() {
    ...
    thread statisticsThread(printStatistics);
